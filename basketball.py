...
while True:
    ...
    screenshot_path = cv.VideoCapture(0)

    # Load the main image and the template images
    main_image = screenshot_path
    # Read a frame from the video capture
    ret, frame = main_image.read()
    ...
    if ret:
        ...
        gray_main = cv.cvtColor(frame, cv.COLOR_BGR2GRAY)
        gray_hoop = hoop_template
        gray_basketball = basketball_template

        # Perform template matching for hoop
        hoop_result = cv.matchTemplate(gray_main, gray_hoop, cv.TM_CCOEFF_NORMED)
        hoop_threshold = 0.8
        hoop_locations = np.where(hoop_result >= hoop_threshold)

        # Perform template matching for basketball
        basketball_result = cv.matchTemplate(gray_main, gray_basketball, cv.TM_CCOEFF_NORMED)
        ...
        if len(hoop_locations[0]) > 0 and len(basketball_locations[0]) > 0:
            ...
            pyautogui.click(basketball_x, basketball_y + 280, 1, 1)
            pyautogui.dragTo(hoop_x, hoop_y + 150, 1, button='left')
            pyautogui.mouseUp()
